...
void MonteCarlo_Hybrid() {
    ...
    futures[0] = std::async(std::launch::async, declaration2, std::ref(A));
    futures[1] = std::async(std::launch::async, declaration2, std::ref(B));
    futures[2] = std::async(std::launch::async, declaration2, std::ref(C));
...
void MonteCarlo_CPUMultiThread(auto &&lambda1, auto &&lambda2, float x1, float x2, float y1, float y2, float z1, float z2) {
    ...
    std::cout << "Max. number of threads: " << max_num_of_threads << std::endl;
    int N_thread = N / max_num_of_threads;
    std::vector<std::future<float>> futures(3);

    for (int n = 0; n < 3; ++n) {
        futures[n] = std::async(std::launch::async, summation, std::ref(lambda1), std::ref(lambda2), std::ref(N_thread), std::ref(x1), std::ref(x2), std::ref(y1), std::ref(y2), std::ref(z1), std::ref(z2));
    }
    auto parallel_result = std::accumulate(futures.begin(), futures.end(), 0.0, [](float acc, std::future<float> &f) {
        return acc + f.get();
    });
...
int main() {
    ...
    if (route) std::cout << "Elapsed time: " << std::chrono::duration_cast<std::chrono::microseconds>(time9 - time8).count() << " milliseconds" << std::endl;
